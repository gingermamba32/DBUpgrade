...
router.post('/radioSearch', function(req,res,next){
	...
	//new RegExp("^" this is for the length only
...
router.post('/addUpc', function(req, res,next){
	...
	Locations.findOne({upcAlias: { $all: req.body.upc} }, function(err, docs) {
		if (docs != null) {
			res.render('invalid', {message: req.body.upc + ' already exists!'});
		}
		else {
...
router.post('/locateThree', function( req, res, next ){
	...
	if ( req.body.upc2 === '' && req.body.quantity2 === '' && req.body.upc3 === '' && req.body.quantity3 === '' && req.body.upc4 === '' && req.body.quantity4 === '' && req.body.upc5 === '' && req.body.quantity5 === ''&& req.body.upc6 === '' && req.body.quantity6 === '' && req.body.upc7 === '' && req.body.quantity7 === '' && req.body.upc8 === '' && req.body.quantity8 === ''){
					...
					Locations.findOne({upcAlias: { $all: req.body.upc3} }, function(err,docs){
							if (docs === null){
								console.log('end here! alias does not mach the upc searched for you will need to add the new UPC to the system!');
								res.render('invalid', {message: req.body.upc3 + ' does not exist. Please add it!'});
							}
							else {
								console.log(docs + 'This is first document to come up with the upc alias.');
								var newLocation = new Locations ({
								...
								console.log(newLocation);
							newLocation.save(function(err, callback){
								res.render('index', {success: req.body.upc1 + ' , ' +req.body.upc2 + ' '+ 'and '+ req.body.upc3 +' have been successfully added to ' + req.body.bin11 + '/PO#' + req.body.shipment});
							})
						}
            		});
					...
					Locations.findOne({upcAlias: { $all: req.body.upc4} }, function(err,docs){
							if (docs === null){
								console.log('end here! alias does not mach the upc searched for you will need to add the new UPC to the system!');
								res.render('invalid', {message: req.body.upc4 + ' does not exist. Please add it!'});
							}
							else {
								console.log(docs + 'This is first document to come up with the upc alias.');
								var newLocation = new Locations ({
		...
		var num5 = Date.now();
					...
					Locations.findOne({upcAlias: { $all: req.body.upc5} }, function(err,docs){
							if (docs === null){
								console.log('end here! alias does not mach the upc searched for you will need to add the new UPC to the system!');
								res.render('invalid', {message: req.body.upc5 + ' does not exist. Please add it!'});
							}
							else {
								console.log(docs + 'This is first document to come up with the upc alias.');
								var newLocation = new Locations ({
					...
					Locations.findOne({upcAlias: { $all: req.body.upc6} }, function(err,docs){
							if (docs === null){
								console.log('end here! alias does not mach the upc searched for you will need to add the new UPC to the system!');
								res.render('invalid', {message: req.body.upc6 + ' does not exist. Please add it!'});
							}
							else {
								console.log(docs + 'This is first document to come up with the upc alias.');
								var newLocation = new Locations ({
					...
					Locations.findOne({upcAlias: { $all: req.body.upc7} }, function(err,docs){
							if (docs === null){
								console.log('end here! alias does not mach the upc searched for you will need to add the new UPC to the system!');
								res.render('invalid', {message: req.body.upc7 + ' does not exist. Please add it!'});
							}
							else {
								console.log(docs + 'This is first document to come up with the upc alias.');
								var newLocation = new Locations ({
					...
					Locations.findOne({upcAlias: { $all: req.body.upc8} }, function(err,docs){
							if (docs === null){
								console.log('end here! alias does not mach the upc searched for you will need to add the new UPC to the system!');
								res.render('invalid', {message: req.body.upc8 + ' does not exist. Please add it!'});
							}
							else {
								console.log(docs + 'This is first document to come up with the upc alias.');
								var newLocation = new Locations ({
...
router.post('/updateall', function(req,res,next){
		       ...
		       var updateObj = req.body;
				...
				Locations.find({description: new RegExp(globalDesc)}).sort({shipment: 1}).exec(function(err, docs) {
					...
					Locations.find({description: new RegExp("^"+globalLength + ".*" + globalColor + "\.$")}).sort({shipment: 1}).exec(function(err, docs){
						console.log( docs + ' good query length color');
						rRadios.find().exec(function(err,files){
					console.log(files);
					res.render('query', {'nums':docs, 'num':files});
				});
					})
